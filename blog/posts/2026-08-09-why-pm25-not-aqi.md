...
The Air Quality Index is not a measurement. It is a **translation** — a way of turning several different pollutants into one 0-to-500 number that a person can act on without knowing any chemistry.

Here is how it works, and where it goes wrong.
...
None of this makes AQI useless. For "should I go for a run this evening?" it is exactly the right tool, and that is what it was designed for. The mistake is letting it stand in for the measurement when the question is anything else.
...
- **The National Clean Air Programme** set its reduction targets against particulate concentrations, not index values.

If you want to know whether your city is meeting the law, whether it is getting better or worse, or whether a minister's claim of "20% improvement" holds — AQI cannot answer any of those. Only the concentration can.

That is the case for leading with PM2.5. Not that AQI is wrong, but that it cannot be checked against anything.
...
Three habits worth having, whether or not you use our site:

**Ask which pollutant.** When you see an AQI, the useful follow-up is "of what?" If the answer is PM2.5, the number means something about long-term health. If it is ozone or NO₂, it is a different problem with different sources and different solutions.

**Ask for the concentration.** Any credible source will give you µg/m³ alongside the index. If a dashboard only shows you an index, it is either simplifying for a reason or hiding something — and you cannot tell which.

**Ask over what period.** A 24-hour figure and an annual mean are not comparable, and the annual one is the one connected to life expectancy. Our [map](https://www.janvayu.in/#map) shows both, and keeps them visibly apart, because collapsing them is the single most common way air-quality numbers get misused in India.
