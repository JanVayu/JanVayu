...
| 32 | **Ward-Level Atlas** | "How Polluted Is Your Ward?" — Leaflet choropleth of every municipal ward across **90 Indian cities** (15 hand-collected + 74 extracted from Swachh Bharat Mission ward boundaries via indianopenmaps.com, `scripts/fetch-openmaps.mjs`, + Guwahati from OpenCity/Oorvani via BharatLas under ODbL-1.0, `scripts/import-bharatlas-wards.mjs`), with live PM2.5 (interpolated) and an **annual satellite PM2.5** layer in **every** city, plus heat (Landsat 8/9 surface temperature), green cover and built-up (ESA WorldCover 10 m) in the **39** cities the raster pipeline has run for so far — satellite layers extracted by `scripts/build-ward-satellite.py`. "Who breathes it" overlays stream schools (UDISE/NCOG) and health centres (Bharatmaps) as vector tiles. Per-layer legend, tooltips and live stats; surfaces the urban heat-island link from each city's own data |
...
**Recently shipped (v26.6.138):** air data reached the whole country. Every one of India's **584,615 villages** is on the map with an **annual mean PM2.5** from satellite (SatPM2.5 V6GL03, ~1 km) — the coverage a ~565-station live network can never give — and the same grid gave the Ward Atlas an **"Air, yearly"** layer, the year-scale partner its heat / green / built-up layers never had. The **Ward Atlas grew 39 → 90 cities / 6,596 wards**: the cap was never air data, it was a hand-written allowlist, so the Swachh Bharat release was mined properly (74 cities) and Guwahati was imported from OpenCity/Oorvani via BharatLas under ODbL, because Swachh Bharat carries no Assam wards at all. Each city now prints its own boundary credit under the map. Ask JanVayu answers village and ward air questions across all of it. Earlier: the indianopenmaps rebuild (Lok Sabha / Vidhan Sabha / district AQI choropleths, pollution-sources overlay, school + health-centre overlays), hand-drawn diagrams, the photo gallery and the site-wide fact-check routine.

**Next up:**

- **Heat / green / built-up for the other 51 ward cities** — the two air layers cover all 90, but the raster pipeline (`scripts/build-ward-satellite.py`) has only been run for 39.
- **Time-aware heat** — seasonal-median land-surface temperature to reduce single-day noise, instead of one clear-sky scene.
- **Ward cities still blocked on data** — West Bengal beyond Kolkata, plus Srinagar and Madurai. Checked and unusable: OpenStreetMap has only 38 `admin_level` 9/10 relations in all of West Bengal and they are villages, not wards; DataMeet covers 31 cities with Kolkata the only Bengal one. Know a source? [contribute@janvayu.in](mailto:contribute@janvayu.in).
- Platform-wide: full WCAG 2.1 AA, longer AQI forecast horizon.

> Two earlier roadmap ideas are now shipped rather than dropped: "no open ward boundaries for Agra, Patna, Surat…" was solved by the SBM ward set via indianopenmaps.com, and satellite-derived per-ward PM2.5 — long marked not feasible because no openly-fetchable ~1 km *live* raster exists — was solved by changing the timescale instead of the resolution: an **annual** ~1 km product does exist openly, and is honest about being annual.
