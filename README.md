...
| 32 | **Ward-Level Atlas** | "How Polluted Is Your Ward?" — Leaflet choropleth of every municipal ward across **97 Indian cities** (15 hand-collected + 74 from Swachh Bharat Mission ward boundaries via indianopenmaps.com, `scripts/fetch-openmaps.mjs`; Guwahati from OpenCity/Oorvani via BharatLas under ODbL-1.0, `scripts/import-bharatlas-wards.mjs`; 7 West Bengal cities from the state's AMRUT GIS master plans, `scripts/import-wb-amrut-wards.mjs`), with live PM2.5 (interpolated) and an **annual satellite PM2.5** layer in **every** city, plus heat (Landsat 8/9 surface temperature), green cover and built-up (ESA WorldCover 10 m) in **every** city — satellite layers extracted by `scripts/build-ward-satellite.py`. "Who breathes it" overlays stream schools (UDISE/NCOG) and health centres (Bharatmaps) as vector tiles. Per-layer legend, tooltips and live stats; surfaces the urban heat-island link from each city's own data |
...
**Recently shipped (v26.6.139):** air data reached the whole country, and then the ward map caught up with it. Every one of India's **584,615 villages** carries an **annual mean PM2.5** from satellite (SatPM2.5 V6GL03, ~1 km) — the coverage a ~565-station live network can never give — and the same grid gave the Ward Atlas an **"Air, yearly"** layer. The atlas grew **39 → 97 cities / 6,936 wards**, and all five layers (live air, annual air, heat, green cover, built-up) now cover every one of them. Boundaries come from four upstreams under four licences, so each city prints its own credit under the map. The last two gaps closed were the ones we had written off: Guwahati, via OpenCity/Oorvani under ODbL, and **seven West Bengal cities** from the state's AMRUT GIS master plans — a dataset sitting in the same release we already used, which we had wrongly reported as non-existent. Ask JanVayu answers village and ward air questions across all of it.

**Next up:**

- **A monthly or seasonal layer** — an annual mean hides the November peak entirely, which for the Indo-Gangetic plain is most of the story. The biggest remaining honesty gap.
- **Time-aware heat** — seasonal-median land-surface temperature to reduce single-day noise, instead of one clear-sky scene. Three cities (Bhopal 34%, Thiruvananthapuram 6%, Kolkata 4%) have wards with no heat value at all because of residual cloud.
- **Ward cities still blocked on data** — Siliguri (its AMRUT upload has 4 of 47 wards), Srinagar and Madurai. The lesson from Bengal is that "no open source" usually means we have not looked hard enough: check every asset in a release, not just the one already wired up. Know a source? [contribute@janvayu.in](mailto:contribute@janvayu.in).
