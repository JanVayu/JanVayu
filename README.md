...
**Recently shipped (v26.6.140):** the ward map went from 39 cities to **142 cities / 9,015 wards** in a week, and all five layers — live air, annual air, heat, green cover, built-up — now cover every one of them. **Every state and union-territory capital is on the map**, including seven that had never had a ward map at all: Srinagar, Agartala, Imphal, Shillong, Itanagar, Aizawl and Kohima. Every one of India's **584,615 villages** carries an annual satellite PM2.5 estimate, and Ask JanVayu can now answer for a **district's villages** as well as a named ward — both were in the data but unreachable by the chatbot until now. The last three expansions all came from the same correction: we had been reading one file out of a data release that contained several, and calling the rest non-existent.
...
- **Thiruvananthapuram's northern 6 wards have no heat value.** Not cloud — a Landsat coverage seam. Verified across 8 pre-monsoon and 6 full-year scenes: every one returns ~5,500 masked pixels with zero valid data. Fixing it means mosaicking two Landsat paths, which the pipeline does not yet do.
- **Ward cities still blocked on data** — Siliguri is the notable one, and now well-evidenced: absent from Swachh Bharat, 4 wards of 47 in the WB AMRUT upload, and absent from the Living Atlas layer. SJDA publishes only mouza (revenue village) PDFs for two rural blocks, which our village layer already covers as vector.
