...
- [x] **The first air layer on the site built from instruments** *(v26.6.184)* — every PM2.5 figure JanVayu carried was modelled: SatPM2.5 V6GL03 for the 2024 map, CAMS bias-corrected onto it for the current year, LongPMInd for 1980–2022. All three are labelled estimates and none is a reading. The [India Air Quality Database](https://airquality.xkdr.org) (XKDR Forum, CC BY 4.0) closes that: 196.5M hourly readings, 558 stations, 15 pollutants. `scripts/build-station-observed.py` → `data/station-observed.json`.
- [x] **And it validates the map we lead with** — for 2024, 534 stations reported PM2.5, **284 cleared twelve complete months** at 75% of hours each, 276 paired to a district. Observed **53.8** against satellite **51.9** µg/m³, **r = 0.834**, RMSE 14.3. The satellite layer reads about 2 µg/m³ lower, which is the direction it should: a district mean averages an urban monitor with the countryside around it. Agreement in pattern, and the metadata says so, because the tempting misreading is that one number corrects the other.
- [x] **Nearly half the network cannot produce a defensible annual mean** — 250 of 534 stations fail the twelve-month test. The count is kept rather than engineered away by loosening the filter.
- [x] **The coverage claim, corrected the day after we made it** *(v26.6.185)* — the archive reports "January 2009 to March 2026" and that is badly misleading. CPCB's feed stops **1 September 2025**, with a hole across January–March 2025; the six months after are two US Embassy monitors. Applying the completeness rule to 2025 leaves one station out of 334. A maximum date is not a coverage claim, and nothing we checked would have caught it.
- [x] **"Was it policy, or was it the wind?" for 44 cities** *(v26.6.186)* — `build-deweathered.py` answered that for Delhi-NCR over 2018–2022 on OpenAQ. `scripts/build-deweathered-national.py` runs the same method (Grange et al. 2018) over CPCB's own hourly record for **44 cities, 2018–2024**. **33 are improving once weather is removed; 11 are not.** Steepest falls are in UP and the western NCR: Meerut −14.63, Varanasi −14.19, Lucknow −13.98 µg/m³/yr. Rising: Chandigarh +3.07, Gwalior +2.37, Chandrapur +2.36, Mumbai +0.80. Delhi −1.78 normalised against −1.75 raw. Held-out R² 0.52–0.91, median 0.81.
...
- [x] **De-weathering** *(done — v26.6.178)* — JanVayu led with annual means partly to sidestep weather, which was honest and was also a limit we stated in public: we could not say whether a city improved because of policy or because of the wind. Closed for Delhi-NCR, for the one window where the station record is continuous.

  **Method:** random forest over wind speed, the two wind vector components, temperature and relative humidity, plus trend, season, day of week and station; normalised by resampling the meteorology while holding time and station fixed (Grange et al. 2018, *Atmos. Chem. Phys.* 18, 6223–6239). Lagged pollutant values are deliberately excluded: feeding yesterday's PM2.5 into a model meant to isolate emissions launders the answer through the target. PM2.5 from 10 CPCB/DPCC/IMD stations via OpenAQ, 7,346 station-days; meteorology from NOAA's Integrated Surface Database at Delhi Safdarjung and Palam.

  **The finding.** Across September and October, the months present in all five years, the measured figures fall by **5.5 µg/m³ a year**. With weather removed only **1.1** remains, about a fifth. Neither clears its 95% interval, so **no direction is claimed** — most of Delhi's apparent improvement in those months was weather, and what is left is not distinguishable from no change.

  **Two traps, both caught before publication.** The first run compared naive annual means and produced a confident *rising* trend of +4.7 µg/m³/yr. That was an artefact: coverage is severely and unevenly seasonal, 2021 is missing April to August (its cleaner months, so it looked terrible at 137.6) and 2022 is missing November and December (its worst, so it looked clean at 82.2). Everything reported is now restricted to months present in every year. The second: OpenAQ's location endpoint advertises 2016 → 2026 for these stations, which is the union of disjoint sensors and not a coverage claim — there is a ~2.3-year ingest gap from late 2022 to early 2025.

  **Validated by placebo.** Shuffling the PM2.5 values against their dates drops held-out R² from 0.749 to **−0.125**, so the pipeline is not leaking. Meteorology alone predicts daily PM2.5 at R² 0.695, and the adjustment removes 72% of the day-to-day variance. `--check` enforces the placebo score in CI and refuses the file if it rises above 0.05.
...
- [x] **PM2.5 made the headline everywhere, after a conference correction** *(v26.6.154)* — someone at a conference said the site led with AQI and PM10 while PM2.5 is what does the damage. They were right. AQI is a unitless index reporting only the worst of six pollutants, scaled differently by CPCB and the US EPA, and meaningless as an annual average — while every Indian limit, every health study and the NCAP targets are in µg/m³ of PM2.5. The walkthrough decks now headline **"Real-time PM2.5 for 157 cities"** (including inside the slide's own diagram, which had the old label drawn in), the dashboard leads with µg/m³, and a new explainer post says why. AQI alerts deliberately unchanged — a same-day index is right for a same-day warning.

- [x] **Ask JanVayu answers land and heat questions** *(v26.6.154)* — `village-stats.json` gained per-district village summaries for surface heat, tree/green/built-up cover and the four seasons (112 KB → 289 KB). "How green is my district" and "when is the air worst here" now get a real number instead of a national average. Three timescales in one record, so each is labelled with its own source and year and the prompt forbids merging them. The green-vs-tree trap is explicit: the typical Indian district is 97% green and 14% treed, so the assistant leads with canopy.

- [x] **A reader's manual for the map** *(v26.6.154)* — [How to Read the JanVayu Map](https://www.janvayu.in/blog/#/posts/2026-08-09-how-to-read-the-map). Everything written about the atlas so far has been engineering: how it was built, what went wrong. Nothing explained how to use it. Two timescales, seven levels, nine measures, three worked questions, and an explicit list of what it cannot tell you.

- [x] **Stale counts swept out of the live site** *(v26.6.154)* — the map description made wards the headline for a seven-level atlas and never mentioned seasons; the live indicator's "27 cities" read as a coverage claim for a platform holding 157 cities and 983,149 areas; the walkthroughs still described the two-screen ward/village model retired in v26.6.145; "115+ cities" sat in the about intro in all five languages. Also: the atlas diagram had never been added to the blog post it was drawn for.

- [x] **Villages carry every metric** *(v26.6.153)* — the last level with air only, and never for want of data: the national passes always scored villages, but a 267 MB tile archive cannot ship so the numbers had nowhere to go. `scripts/build-village-layers.py` writes them into the per-district TopoJSON the village loader already fetches. **584,586 of 584,615 (100.00%)** — air and all four seasons on every one, land cover 99.98%, heat 99.87%.

  **Joining them was the work.** The LGD code looks like the key and is not one twice over. **4,856 codes are carried by more than one polygon** and those polygons genuinely differ — 645088 covers one shape 12% treed and another 67% — so shared codes are resolved by centroid. And **37,563 villages have no code at all**, blank on both sides; code matching reached 93.57% and left 6.4% of the country as grey holes. Those are matched by geometry against a 2 km grid of the codeless records, each claimed once, within 3 km.

  **The join checks itself, which is why it can be trusted.** Both files hold an annual PM2.5 computed independently from the same grid — one sampled from the TopoJSON polygon, one from the boundary polygon — so `--verify` compares them. Median difference **0.00 µg/m³** on both paths; 0.023% of code matches and 0.072% of geometry matches differ by more than 2 µg/m³. The fallback is as reliable as the key it replaces.

  **Two key collisions, renamed rather than papered over.** A village's `s` was its state name and `s` is summer air; a district's `b` in `_index.json` is its bounding box, which the map uses to decide what to fetch, and `b` is built-up. State moved to `st` and the per-district means into a nested `m`. Writing over either would have failed silently, and the second would have broken the village layer outright. The rename is guarded on the value's *type*, because running the script twice would otherwise have moved summer air into every state name.

  Cost: `data/villages` 155 MB → 192 MB.

- [x] **ULB duplicates** *(v26.6.153)* — the same scan found **9 duplicate geometries at ULB level**; `dedupe-ward-atlas.py` handles any level via `--in`/`--out`, so it took one run. **3,368 → 3,359 cities.** Coverage recomputed on the deduplicated set: heat 99.88%, land cover 99.97%.
- [x] **Ward panel retired, correlation moved onto the map** *(v26.6.151)* — the panel was a dropdown over 142 cities, each with its own baked GeoJSON. Every layer it had now covers the whole country on the map, so it goes.
  ...
  The `#ward-map` route still resolves, to a page pointing at the map, so existing links and bookmarks do not break.

- [x] **The panel's JS removed, and a live bug found doing it** *(v26.6.153)* — **536 lines** gone from `app.js`, plus 16 lines of orphaned CSS. The note left here said parts of it were reached by Ask JanVayu and the share-card path; checking rather than trusting that note, nothing outside the panel reached any of it. What *was* shared is the schools / health-centre overlay, and it was broken: `toggleWardReceptor` ended with `if (!wardMap || store[kind]) return;`, and `wardMap` has been permanently `null` since the panel was retired, so on the live map both buttons lit up and added no layer. Confirmed by clicking them in a real browser against `main` (receptor pane count 0) and after the fix (1). Renamed to `toggleReceptorLayer`, since it has nothing to do with wards any more.
