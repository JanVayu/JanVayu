...
**Follow-ups still open after this phase:** a monthly or seasonal layer for both villages and wards, since an annual mean hides the November peak entirely; **more Ward Atlas cities** — *batch 1 shipped in v26.6.137, taking the atlas 39 → 67 cities / 5,702 wards; batch 2 (~28 more) is queued.* The cap was never air data, which now covers the whole country — it was a hand-written allowlist, against a source holding 3,675 ULBs. West Bengal, Manipur, Mizoram and Tripura are absent from the SBM ward release, so no city in those states can be added by the automated pipeline (Kolkata is in the atlas only because its 141 wards were hand-collected from another source). OpenStreetMap `admin_level` relations, DataMeet, and state municipal portals are the next places to look; and the repo weight question — the working tree is now ~182 MB, so a contributor-friendly shallow-clone or data-split path is worth considering.
