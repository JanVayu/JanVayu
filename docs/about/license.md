# License

JanVayu uses a **dual licence** to distinguish between its code and its content.

---

## Code: MIT License

All source code in this repository (HTML, CSS, JavaScript, Netlify Functions) is licensed under the **MIT License**.

This means you are free to:
- Use the code for any purpose, including commercial purposes
- Modify and distribute the code
- Incorporate it into other projects

The only requirement is to retain the copyright notice.

**Full text:** [LICENSE](https://github.com/JanVayu/JanVayu/blob/main/LICENSE) file in the repository root.

---

## Content: CC BY-NC-SA 4.0

All documentation, written content, research summaries, and data presented on the platform and in this documentation are licensed under **Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International**.

This means you are free to:
- **Share** — copy and redistribute the material in any medium or format
- **Adapt** — remix, transform, and build upon the material

Under the following terms:
- **Attribution (BY)** — give appropriate credit to JanVayu and link to the original
- **NonCommercial (NC)** — you may not use the material for commercial purposes
- **ShareAlike (SA)** — if you remix or adapt, distribute under the same licence

**Full text:** [creativecommons.org/licenses/by-nc-sa/4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/)

---

## Data Sources

Data integrated into JanVayu from external sources retains the original licence of each source. JanVayu does not claim ownership of third-party data. When using data from JanVayu, cite the original primary source (CPCB, Lancet, World Bank, etc.), not JanVayu.

---

## Citation

To cite JanVayu in academic or policy work:

```
JanVayu. (2026). India's Citizen Air Quality Platform. MMSF Air Quality Initiative. 
Retrieved from https://www.janvayu.in
```
