...
// A rural / village question, as opposed to a city or ward one. Land cover and
// heat widened this: "are there any trees in my district" is a village question
// even though it never says "air".
function isVillageQuery(question) {
  ...
  return /\b(village|villages|rural|gaon|gram|panchayat|taluk|tehsil|block|countryside|my district|district)\b/.test(q)
    || /\b(tree|trees|canopy|forest|green cover|greenery|built[- ]up|land cover|surface heat|heat island)\b/.test(q);
}
...
function buildVillageContext(question) {
  ...
  // Air by season. The single most useful thing here for a rural reader: an
  // annual mean hides a factor-of-three swing in the Indo-Gangetic plain, and
  // "when is it worst" is a question the annual figure actively obscures.
  if (d.seasons) {
    const S = d.seasons;
    const names = { w: 'winter (Dec–Feb)', s: 'summer (Mar–May)', r: 'monsoon (Jun–Sep)', o: 'post-monsoon (Oct–Nov)' };
    const order = Object.entries(S).sort((a, b) => b[1] - a[1]);
    b += `\n\nTHE SAME VILLAGES BY SEASON (2024 satellite, µg/m³ — village averages for this district):`;
    b += `\n   - Winter ${S.w} · summer ${S.s} · monsoon ${S.r} · post-monsoon ${S.o}.`;
    b += `\n   - Worst season here is ${names[order[0][0]]} at ${order[0][1]}; cleanest is ${names[order[3][0]]} at ${order[3][1]} — a ${(order[0][1] / Math.max(order[3][1], 0.1)).toFixed(1)}× swing that the annual mean of ${d.mean} hides.`;
    b += `\n   - Still ANNUAL-SCALE data, four seasonal means rather than daily readings. Never present a seasonal mean as a forecast or as today's air.`;
  }

  // Land cover and surface heat. Different sources AND different years from
  // the air figures, so they get their own block and their own dates. The
  // green/tree distinction is the important one to get right: green counts
  // cropland, which is most of rural India, so it is near 100 nearly
  // everywhere and says almost nothing on its own.
  if (d.tree || d.heat) {
    b += `\n\nLAND AND HEAT IN THE SAME DISTRICT'S VILLAGES — different sources and different years from the air figures above, so keep them apart:`;
    if (d.tree) b += `\n   - Tree cover (ESA WorldCover 2021, 10 m, canopy only): typical village ${d.tree.med}%, average ${d.tree.mean}%.`;
    if (d.green) b += `\n   - Green cover: typical village ${d.green.med}%. CAUTION: "green" counts cropland, shrub and grass as well as trees, so in rural India it is near 100% almost everywhere and is NOT evidence of greenery in the everyday sense. If someone asks how green their area is, answer with TREE COVER and explain the difference.`;
    if (d.built) b += `\n   - Built-up land: typical village ${d.built.med}%, average ${d.built.mean}%.`;
    if (d.heat) b += `\n   - Surface heat (Landsat 8/9, mean land-surface temperature, 2026 pre-monsoon): average ${d.heat.mean}°C across ${d.heat.n} villages. This is how hot the GROUND gets under a clear pre-monsoon sky — it runs well above the air temperature a forecast would give, and is not a weather reading.`;
    if (d.treeExt) b += `\n   - Most treed village: ${d.treeExt.most.n} (${d.treeExt.most.t}% canopy). Least: ${d.treeExt.least.n} (${d.treeExt.least.t}%).`;
    b += `\n   - Nationally, tree cover tracks surface heat (r = −0.43): the treeless places are the hot ones. Green cover does not, for the cropland reason above.`;
  }
  return b;
}
...
6) The INSTRUMENT RECORD, and how it checks the modelled layers (JanVayu's own working, 2024)
   - JanVayu's wide-coverage air layers are MODELLED on purpose, because they reach where monitors do not: SatPM2.5 V6GL03 gives all 584,615 villages and 68,596 wards a 2024 annual figure, CAMS carries it to the current year, LongPMInd reaches back to 1980. Roughly 565 continuous CPCB stations cannot do that and never will. None of that is a stopgap; never describe the modelled layers as second-best.
   - What the site now ALSO holds is the measured annual record, so the models can be checked against instruments: the India Air Quality Database (XKDR Forum, airquality.xkdr.org, CC BY 4.0), compiling CPCB CAAQM plus US Department of State monitors via AirNow.
   - 2024: 534 stations reported. 284 passed the completeness rule (all twelve months present, each with at least 75% of its possible hours); 276 of those sit within 50 km of a district centroid and were paired with the satellite figure for that district.
...
function isTrendQuery(question) {
  return /\b(trend|history|historical|over time|past (year|month|5 year|decade)|since 20\d\d|getting (better|worse|cleaner|dirtier)|year ?over ?year|yoy|2019 vs|compared to (previous|last) year|annual average|improv(e|ed|es|ing|ement)|worsen(ing|ed)?|deteriorat\w*|de-?weather\w*|normalis(ed|ation)|meteorolog\w+ normalis\w+|policy or (the )?(weather|wind)|(weather|wind) or policy|actually (better|cleaner|improving|working)|(is|has|are) (ncap|grap|the polic(y|ies)|the plan|it|they) (been )?work(ing|ed)?)\b/i.test(question);
}
...
function buildSystemPrompt(seasonal, lang, opts = {}) {
  const { nationalQuery = false, multiSource = false, topical = false, methodologyNeeded = false,
    ward = false, stationCount = false, rti = false, stationRange = false, village = false,
    trend = false } = opts;
  ...
  // v26.6.124 — Situational instructions: only sent when the request actually
  // needs them (~770 tokens trimmed from the common-case prompt, ~30-40% less),
  // so more questions fit inside Groq's per-minute token budget. The core
  // safety / tone / anti-fabrication rules further down are ALWAYS included.
  const i12 = (nationalQuery || topical) ? `\n12. For NATIONAL/TOPICAL questions (EVs, low-cost sensors, BS-VI, monitoring network, court orders, NCAP): use the TOPICAL REFERENCE block when provided. Do NOT default to Delhi or single-station context unless the user explicitly asks about Delhi.` : "";
...
5 cleanest: ${bottom5}`;
  ...
  const stationCount = isStationCountQuery(question);
  const topical = nationalQuery || stationCount || isApportionmentQuery(question) || isRankingQuery(question);
  const methodologyNeeded = multiSource;
  // v26.6.124 — gate situational instruction rules (ward/RTI/station) to shrink
  ...
  try {
    const groqRes = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: GROQ_MODEL,
        messages: [
          { role: "system", content: buildSystemPrompt(seasonal, requestedLang, promptOpts) },
